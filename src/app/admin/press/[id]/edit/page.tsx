  ...
  useEffect(() => {
    fetch(`/api/press/${id}`).then(r => r.json()).then(data => {
      setForm({
        ...
        author: data.author ?? "NeoLAB", category: data.category ?? "press",
        externalUrl: data.externalUrl ?? "", tags: data.tags ?? "",
        file1Url: data.file1Url ?? "", file1Name: data.file1Name ?? "",
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl p-8 w-[360px] flex flex-col items-center gap-5">
            ...
            <h2 className="text-lg font-bold text-gray-800">기업뉴스 수정</h2>
          ...
          <table className="w-full text-sm">
            <tbody>
              ...
              <tr className="border-b border-gray-100">
                ...
                <td className="px-5 py-3">
                  <input value={form.titleKo} onChange={(e) => set("titleKo", e.target.value)} required placeholder="기사 제목"
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1a1a2e]" />
                </td>
              </tr>
              <tr className="border-b border-gray-100">
