...
export default function NewNoticePage() {
  ...
  async function uploadFile(file: File, slot: 1 | 2): Promise<{ url: string; name: string }> {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const payload = { ...form };
      if (file1) {
        const { url, name } = await uploadFile(file1, 1);
        payload.file1Url = url;
        payload.file1Name = name;
      }
      if (file2) {
        const { url, name } = await uploadFile(file2, 2);
