...
export default function AdminCustomerPage() {
  const [items, setItems] = useState<CustomerNotice[]>([]);
  ...
  const fetchAll = useCallback(async () => {
    ...
    const list: CustomerNotice[] = Array.isArray(data) ? data : [];
  ...
  const nonPinnedFiltered = useMemo(() => filtered.filter((x) => !x.isPinned), [filtered]);
    ...
    <AdminPage>
      ...
      <AdminHeaderRowBetween>
        ...
        <AdminFlexGap2>
          ...
          <AdminLinkPrimary href="/admin/customer/new">글쓰기</AdminLinkPrimary>
        </AdminFlexGap2>
      </AdminHeaderRowBetween>
                ...
                pagedItems.map((n) => {
                  ...
                  return (
                    <AdminTr key={n.id} $hover $selected={selected.has(n.id)}>
                      ...
                      <AdminTd $align="center">
                        {n.isPinned ? (
                          <AdminNoticePinBadge>공지</AdminNoticePinBadge>
                        ) : (
                          <AdminMutedSmall as="span">{nonPinnedFiltered.length - nonPinnedIdx}</AdminMutedSmall>
                        )}
                      </AdminTd>
                      <AdminTd>
                        <AdminTitleLink href={`/admin/customer/${n.id}/edit`}>
                          {n.titleKo || "(제목 없음)"}
                        </AdminTitleLink>
                      </AdminTd>
                      <AdminTd $align="center">
                        <AdminCellHCenter>
                          <AdminAuthorBadge $color={badgeColor(n.author)}>
                            {n.author?.charAt(0) || "?"}
                          </AdminAuthorBadge>
                          <AdminMutedSmall as="span">{n.author}</AdminMutedSmall>
                        </AdminCellHCenter>
                      </AdminTd>
                      <AdminTd $align="center">
                        <AdminMutedSmall as="span">{fmtDate(n.createdAt)}</AdminMutedSmall>
                      </AdminTd>
                      <AdminTd $align="center">
                        <AdminMutedSmall as="span">{n.views ?? 0}</AdminMutedSmall>
                      </AdminTd>
                    </AdminTr>
                  );
                })
