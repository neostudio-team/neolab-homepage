...
export default function AdminCustomerPage() {
  ...
  async function handleDelete(id: string) {
    ...
    await fetch(`/api/customer-notices/${id}`, {
      method: "DELETE",
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-800">고객지원 공지 관리</h1>
        <Link href="/admin/customer/new" className="bg-[#1a1a2e] text-white px-4 py-2 rounded-lg text-sm hover:bg-[#16213e] transition-colors">
          + 새 글 작성
        </Link>
      </div>
              ...
              {notices.map((n) => (
                <tr key={n.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 flex gap-2">
                    <Link href={`/admin/customer/${n.id}/edit`} className="text-blue-600 hover:underline">수정</Link>
