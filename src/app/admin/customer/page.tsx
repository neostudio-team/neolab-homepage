...
export default function AdminCustomerPage() {
  ...
  async function doDelete() {
    ...
    await Promise.all([...selected].map(id =>
      fetch(`/api/customer-notices/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } })
    ));
  ...
  return (
    <div className="p-8 max-w-6xl">
      ...
      <h1 className="text-2xl font-bold text-gray-800 mb-6">고객지원 공지 관리</h1>
      ...
      <div className="flex items-center justify-between mb-2 px-1">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/admin/customer/new" className="px-4 py-1.5 bg-[#1a1a2e] text-white rounded-lg text-xs hover:bg-[#16213e] transition-colors">
            글쓰기
          </Link>
        </div>
      </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {pagedItems.length === 0 ? (
                <tr><td colSpan={6} className="px-6 py-12 text-center text-gray-400 text-sm">등록된 공지가 없습니다.</td></tr>
              ) : pagedItems.map(n => {
                ...
                return (
                  <tr key={n.id} className={`hover:bg-gray-50 transition-colors ${selected.has(n.id) ? "bg-blue-50/60" : ""}`}>
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/admin/customer/${n.id}/edit`} className="text-gray-800 hover:text-[#1a1a2e] hover:underline font-medium line-clamp-1">{n.titleKo || "(제목 없음)"}</Link>
                    </td>
                    <td className="px-4 py-3 text-center">
